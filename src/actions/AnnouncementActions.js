...
export const postAnnouncement = ({ classId, content }) => {
    return (dispatch) => {
        ...
        axios.post(`http://localhost:5000/announcement/teacher/${classId}`, {content})
        .then(res => {
...
export const getAnnouncementData = (classId) => {
    return (dispatch) => {
            axios.get(`http://localhost:5000/announcement/teacher/${classId}`)
            .then((res)=> {
...
export const getStudentAnnouncementData = (classId) => {
    return (dispatch) => {
            axios.get(`http://localhost:5000/announcement/student/${classId}`)
            .then((res)=> {
                dispatch(fetchAnnouncementData(res.data)) 
            })
            .catch((error) => {
